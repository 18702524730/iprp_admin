angular.module('iprpAdminApp').controller('CaseListCtrl',function($scope,ipService,ChannelService,PaginationService,$location,$state, session, $modal){
  $('#case_list').addClass("selected").parent().siblings().children().removeClass("selected");
  ...
  function init_order_list() {
      ...
      var OweNer = function (pgIndex, pgCount, cb) {
          ...
          $location.path('main/ip_protect_home/case_list').search(object);
          ipService.findSafeguardCompanyCaseList(object, cb);
      };
  ...
  $scope.detail_orders = function(index){
      $location.path('main/ip_protect_home/case_detail/' + index);
  }
