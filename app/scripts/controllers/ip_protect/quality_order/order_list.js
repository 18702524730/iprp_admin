angular.module('iprpAdminApp').controller('QualityListCtrl',function($scope,ipService,ChannelService,PaginationService,$location,$state, session, $modal){
  $('#quality_list').addClass("selected").parent().siblings().children().removeClass("selected");
  ...
  function init_order_list() {
      ...
      var OweNer = function (pgIndex, pgCount, cb) {
          ...
          $location.path('main/ip_protect_home/quality_list').search(object);
          ipService.findStereoProtectOrderList(object, cb);
      };
  ...
  $scope.detail_orders = function(index){
      $location.path('main/ip_protect_home/quality_detail/' + index);
  }
