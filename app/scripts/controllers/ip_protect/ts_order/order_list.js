angular.module('iprpAdminApp').controller('TsListCtrl',function($scope,ipService,ChannelService,PaginationService,$location,$state, session, $modal){
  $('#ts_list').addClass("selected").parent().siblings().children().removeClass("selected");
  ...
  function init_order_list() {
      ...
      var OweNer = function (pgIndex, pgCount, cb) {
          ...
          $location.path('main/ip_protect_home/ts_list').search(object);
  ...
  $scope.detail_orders = function(index){
      $location.path('main/ip_protect_home/ts_detail/' + index);
  }
