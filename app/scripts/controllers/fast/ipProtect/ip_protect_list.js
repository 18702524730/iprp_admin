angular.module('iprpAdminApp').controller('IpProtectCtrl',function($scope,ipProtectService,PaginationService,$location,$state, session,$interval){
   $('#ip_protect_list').addClass("selected").parent().siblings().children().removeClass("selected");
  ...
  $scope.viewDetail = function(item, isPop, isAdd){
    ...
    $location.path('main/fast_home/ip_protect_detail/'+item.consultationId).search(ret);
  };
  ...
  function init_order_list(needInit) {
      ...
      var OweNer = function (pgIndex, pgCount, cb) {
        ...
        var object = $.extend({}, $scope.obj, pageConfig);
        $location.path('main/fast_home/ip_protect').search(object);
        ipProtectService.list(object, cb);
      };
      $scope.pagination = new PaginationService(OweNer, 15, $scope.obj.pgIndex, true);
