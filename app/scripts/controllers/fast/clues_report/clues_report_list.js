angular.module('iprpAdminApp').controller('cluesReportCtrl',function($scope,ipProtectService,PaginationService,$location,$state, session,$interval){
   $('#clues_report_list').addClass("selected").parent().siblings().children().removeClass("selected");
  ...
  $scope.viewDetail = function(item, isPop, isAdd){
    ...
    $location.path('main/fast_home/clues_report_detail/'+item.consultationId).search(ret);
  };
  ...
  function init_order_list(needInit) {
      ...
      var OweNer = function (pgIndex, pgCount, cb) {
        ...
        var object = $.extend({}, $scope.obj, pageConfig);
        $location.path('main/fast_home/clues_report').search(object);
        ipProtectService.clueReportList(object, cb);
      };
      $scope.pagination = new PaginationService(OweNer, 15, $scope.obj.pgIndex, true);
