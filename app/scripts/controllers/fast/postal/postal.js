angular.module('iprpAdminApp').controller('postalCtrl',function($scope,$modal,PostalService,$stateParams,$location,$state,Upload,session,PaginationService){
  $('#postal').addClass("selected").parent().siblings().children().removeClass("selected");
  // 添加时间控件 初始化
  ...
  $scope.apply_Info = function(){
      ...
      $location.path('main/fast_home/postal').search(object);
      PostalService.list(object, cb);
    };
  ...
  $scope.edit= function(item){
    $modal.open({
        ...
        backdrop:false,
        keyboard:true,
        templateUrl: 'package\\modal\\fast\\editPostal.html',
        controller: 'editPostalCtrl'
