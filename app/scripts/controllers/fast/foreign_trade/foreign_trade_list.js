angular.module('iprpAdminApp').controller('foreignTradeCtrl',function($scope,$modal,ServicerService, FeedbackService,PaginationService,$stateParams,$location,Upload,$state){
  ...
  function init_order_list() {
      ...
      var OweNer = function (pg_index, pg_count, cb) {
          ...
          FeedbackService.findZjForeignTradeIPProblemList(object, cb);
  ...
  $scope.detail_orders = function(item){
    // $state.go('main.fast_home.foreign_trade_detail', {id: item.id});
    $location.path('main/fast_home/foreign_trade_detail/'+item.id)
  };
