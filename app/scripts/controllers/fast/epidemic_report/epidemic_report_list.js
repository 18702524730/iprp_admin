angular.module('iprpAdminApp').controller('epidemicReportCtrl',function($scope,ipProtectService,PaginationService,$location,$state, session,$interval){
   $('#epidemic_report_list').addClass("selected").parent().siblings().children().removeClass("selected");

  //查询接待员
  /*ipProtectService.findAllReceptionName(function(data){
  ...
  $scope.viewDetail = function(item, isPop, isAdd){
    ...
    $location.path('main/fast_home/epidemic_report_detail/'+item.consultationId).search(ret);
  };
  ...
  //       if ($scope.obj.pgIndex) {
  //         pgIndex = $scope.obj.pgIndex;
  //         $scope.obj.pgIndex = '';
  //       }
  //       var pageConfig = {
  //           "pgIndex":pgIndex,
  ...
  //       $location.path('main/fast_home/ip_protect').search(object);

  //       ipProtectService.list(object,function(data){
  //         $scope.OrderList = data.elements;
  //       });
  //     };
  //     $scope.pagination = new PaginationService(OweNer, 15, $scope.obj.pgIndex, true);
  // }
  init_order_list();
  ...
  function init_order_list(needInit) {
      ...
      var OweNer = function (pgIndex, pgCount, cb) {
        ...
        var object = $.extend({}, $scope.obj, pageConfig);
        $location.path('main/fast_home/epidemic_report').search(object);
        ipProtectService.epidemicReportList(object, cb);
      };
      $scope.pagination = new PaginationService(OweNer, 15, $scope.obj.pgIndex, true);
