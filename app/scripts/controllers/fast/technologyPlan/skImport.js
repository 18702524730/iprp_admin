angular.module('iprpAdminApp').controller('SkImportCtrl', function ($scope,$modal,Enterprises,PaginationService,$location,session,Upload) {
  $('#skImport').addClass("selected").siblings().removeClass("selected");
  $scope.obj = {};
  ...
  function init_seller_list () {
    ...
    var OweNer = function(pg_index,pg_count,cb) {
      ...
      Enterprises.queryShengkeList(params,cb);
    };
  ...
  $scope.uploadFile = function(file){
  	if (!file) {return}
  	Upload.upload({
        url: options.api.base_url + '/tech/importShengkeExcel?access_token=' + session.accessToken,
        data: {file: file}
    }).then(function (resp) {
    	layer.alert('导入成功');
    	$scope.search();
