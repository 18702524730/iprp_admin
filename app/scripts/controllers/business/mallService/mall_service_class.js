angular.module('iprpAdminApp').controller('mallServiceClassCtrl', function ($scope,BusinessClassService,PaginationService,$location) {

    $('#mall_service_class').addClass("selected").parent().siblings().children().removeClass("selected");
    ...
    $scope.edit_class=function(index,name){
      $location.path('/main/business_home/edit_mall_service_class/'+index);
    };
    //添加
    $scope.addChild_class=function(bs_class_id,name){
      ...
      layer.open({
        ...
        // localStorage.setItem("mall_bs_class_id", bs_class_id);
        // $scope.temp = localStorage.getItem('mall_bs_class_id');
        // $location.path('/main/business_home/add_mall_service_class');
    };
    //不需要上级添加
    $scope.add_business_class = function(){
        localStorage.removeItem('mall_bs_class_id');
        $location.path('/main/business_home/add_mall_service_class');
    }
    //删除
