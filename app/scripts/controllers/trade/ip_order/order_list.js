angular.module('iprpAdminApp').controller('IpOrderListCtrl',function($scope,ipService,ChannelService,PaginationService,$location,$state, session, $modal){
  $('#ip_order_list').addClass("selected").parent().siblings().children().removeClass("selected");
  ...
  function init_order_list() {
      ...
      var OweNer = function (pgIndex, pgCount, cb) {
          ...
          $location.path('main/trade_home/ip_order_list').search(object);
  ...
  $scope.detail_orders = function(index){
      $location.path('main/trade_home/ip_order_detail/' + index);
  }
