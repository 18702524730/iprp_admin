angular.module('iprpAdminApp').controller('BsOrderListCtrl',function($scope,bsOrderService,ChannelService,PaginationService,$location,$state,session){
    $('#bs_order_list').parent().siblings().children('a').removeClass("selected");
    $('#bs_order_list').addClass("selected");
  ...
  // 添加时间控件 初始化
    ...
    $scope.detail_orders = function(index){
        $location.path('main/trade_home/bs_order_detail/' + index);
    }
