angular.module('iprpAdminApp').controller('TmallNiceListCtrl',function($scope,bsOrderService,ChannelService,PaginationService,$location,$state,session){
    $('#tmall_nice_order').addClass("selected").parent().siblings().children('a').removeClass("selected");

  // 添加时间控件 初始化
    $('#nicecreateTimeStart,#nicecreateTimeEnd').datetimepicker({
    ...
    function init_order_list() {
        ...
        var OweNer = function (pg_index, pg_count, cb) {
            ...
            bsOrderService.tmall_nice_list(object, cb);
            var o = $.extend({}, object);
            $location.path('main/trade_home/tmall_nice_list').search(o);
        };
