angular.module('iprpAdminApp').controller('DemandOrderListCtrl',function($scope,demandService,PaginationService,$location,$state, session){

  $('#demand_list').parent().siblings().children('a').removeClass("selected");
  ...
  function init_order_list() {
      ...
      var OweNer = function (pg_index, pg_count, cb) {
          var pageConfig = {
              "pgIndex": pg_index || 1,
              "pgCount": pg_count,
              "requirementTimeStart": get_startCreateTime (),
              "requirementTimeEnd": get_endCreateTime (),
          ...
          demandService.partnerList(object, cb);
      };
  ...
  $scope.detail_orders = function(index){
      $location.path('main/trade_home/demand_detail/' + index);
  }
