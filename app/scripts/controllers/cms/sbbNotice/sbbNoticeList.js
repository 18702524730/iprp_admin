angular.module('iprpAdminApp').controller('SbbNoticeListCtrl',function($scope,$modal,SbbService,PaginationService,$stateParams,$location,Upload,$state){
...
$scope.toTop = function (id, sort) {
	if (sort == 1) {
		SbbService.cancelSetTopeMarketing({id, category: 2}, function(data){
			layer.msg('操作成功');
			$scope.obj.pg_index = 0;
			init_order_list();
		});
	...
	SbbService.setTopeMarketing({id, category: 2}, function(data){
		layer.msg('操作成功');
		$scope.obj.pg_index = 0;
		init_order_list();
	});
...
$scope.goDetailFn = function(id) {
  $state.go('main.cms_home_list.addNotice', {id: id});
};
