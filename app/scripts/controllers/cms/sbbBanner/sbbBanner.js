angular.module('iprpAdminApp').controller('SbbBannerCtrl',function($scope,$modal,SbbService,PaginationService,$stateParams,$location,Upload,$state){
  $('#sbb_banner').addClass("selected").parent().siblings().children().removeClass("selected");
    // 添加时间控件 初始化
    $('#startCreateDate,#startDate,#endDate,#endCreateDate').datetimepicker({
  ...
  function init_order_list() {
      ...
      var OweNer = function (pg_index, pg_count, cb) {
          ...
          if ($scope.obj.startEffectiveStartTime) {
            $scope.obj.startEffectiveStartTime = new Date($scope.obj.startEffectiveStartTime).getTime();
          }
          if ($scope.obj.endEffectiveStartTime) {
            $scope.obj.endEffectiveStartTime = new Date($scope.obj.endEffectiveStartTime).getTime() + 24*3600*1000-1;
          }
...
$scope.delItem = function (id) {
  ...
  $scope.goDetailFn = function(id) {
    $state.go('main.cms_home_list.addBanner', {id: id});
  };
