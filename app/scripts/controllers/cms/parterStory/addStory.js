angular.module('iprpAdminApp').controller('AddStoryCtrl',function($scope,$modal,SbbService,PaginationService,$stateParams,$location,Upload,$state){
  $('#sbb_story').addClass("selected").parent().siblings().children().removeClass("selected");

  $scope.obj = {author: '',title: '', summary: '', category: 4, params: '', jumpType: 2};
  ...
  $scope.addGoodClick = function () {
    ...
    console.log('content', editor.txt.html())
    // 富文本内容
    if (editor.txt.html() === '<p><br></p>') {
        layer.msg('请输入详情内容')
        return;
