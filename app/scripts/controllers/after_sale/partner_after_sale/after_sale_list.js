angular.module('iprpAdminApp').controller('PartnerAfterSaleListCtrl',function($scope,bsOrderService,PartnerService,RefundService,PaginationService,$location,$state){
    ...
    function init_refund_list() {
      ...
      //获取时间  end_time
        ...
        var OweNer = function (pg_index, pg_count, cb) {
            var ret = {
              "refund_sn":$scope.obj.refund_sn === undefined ? null : $scope.obj.refund_sn,//退款单号
              ...
              "state":$scope.obj.state,//退款状态
              "sp_state":$scope.obj.state,//服务状态
              "pg_index": pg_index,
              "pg_count": pg_count,
              "asistantPhone": $scope.obj.asistantPhone === undefined ? null : $scope.obj.asistantPhone,
              "orderStoreName": $scope.obj.orderStoreName === undefined ? null : $scope.obj.orderStoreName,
            };
            console.log(PartnerService)
            PartnerService.list(ret, cb);
            var o = $.extend({}, ret, $scope.typeobj);
            $location.path('main/after_sale_home/after_sale_list').search(o);
        };
  ...
  //重置
    ...
    $scope.orderDetail = function(list){
        $location.path('/main/after_sale_home/after_sale_detail/' + list.refund_id);
    };
    $scope.orderServiceDetail = function(list){
