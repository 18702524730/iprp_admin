angular.module('iprpAdminApp').controller('UpdateCheckCtrl',function($scope,$stateParams,RefundService,RefundChildService,$location){
    ...
    $scope.check_payment = function(ind,order_pay_id){
        if($scope.check_ids.indexOf(order_pay_id) !== -1){
            $scope.check_ids.splice($scope.check_ids.indexOf(order_pay_id),1);
            get_order_pay_detail('splice',ind,order_pay_id);
        }else{
            $scope.check_ids.push(order_pay_id);
            get_order_pay_detail('push',ind,order_pay_id);
            $scope.payment_error_manage = "";
        }
    };
    function get_order_pay_detail(msg,ind,order_pay_id){
        RefundChildService.get({
            order_pay_id:order_pay_id,
            refund_id:$stateParams.refund_id
        },function(data){
            $scope.refund_child_detail = data;
            if($scope.refund_child_detail != undefined){
                if(msg === 'push'){
                    $scope.refund_apply_obj.id = $scope.refund_child_detail.id;
                    $scope.refund_apply_obj.refund_id = $scope.refund_child_detail.refund_id;
                    $scope.refund_apply_obj.order_pay_id = $scope.refund_child_detail.order_pay_id;
                    $scope.refund_apply_obj.payment_sn = $scope.refund_child_detail.payment_sn;
                    $scope.refund_apply_obj.payment_pe_amount = $scope.refund_child_detail.payment_pe_amount;
                    $scope.refund_apply_obj.payment_sp_amount = $scope.refund_child_detail.payment_sp_amount;
                    $scope.refund_apply_obj.refund_child_real_pe_amount = $scope.refund_child_detail.refund_child_real_pe_amount;
                    $scope.refund_apply_obj.refund_child_real_sp_amount = $scope.refund_child_detail.refund_child_real_sp_amount;
                    $scope.refund_apply_obj.payment_description = $scope.refund_child_detail.payment_description;
    ...
    function save_add_refund(){
        ...
        if(sp_flag === false && pe_flag === false){
            if(!$scope.refund_detail.return_message){
                ...
                RefundService.update_refund({refund_id:$stateParams.refund_id},$scope.refund_detail,function(data){
