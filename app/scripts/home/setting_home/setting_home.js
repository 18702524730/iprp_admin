angular.module('iprpAdminApp').controller('SettingHomeCtrl', function ($scope,session,$location) {
    $('#nav_setting').addClass("actived").parent().siblings().find('a').removeClass("actived");
    //点击改变左边的class
    $scope.select=function($event,message){
        ...
        if(message === "setting"){
            ...
            if(session.email_manage_show === false) {
                if (session.email_tpl_list_show === false) {
                    $location.path('/main/setting_home/email_tpl');
                    return;
                } else if (session.email_set_show === false) {
                    $location.path('/main/setting_home/email');
                    return;
                }
                return;
            }
